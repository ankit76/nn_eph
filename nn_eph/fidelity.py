...
import jax
from jax import random, lax, vmap, hessian, tree_util, grad, vjp, value_and_grad, jit, numpy as jnp
...
class wavefunction():
    ...
    def calc_overlap(self, parameters, input):
        ...
        overlap = jnp.exp(output_r[0]) * jnp.exp(1.j * jnp.sum(output_phi))
...
def calc_fidelity(parameters, psi, psi_target, basis):
    ...
    fidelity = 1 - jnp.abs(jnp.sum(psi_overlaps * jnp.conj(psi_target))) ** 2 / (jnp.sum(jnp.abs(psi_overlaps) ** 2) * jnp.sum(jnp.abs(psi_target) ** 2))
    return fidelity
...
def optimize(optimizer, parameters, psi, psi_target, basis, max_iter=1000):
    ...
    def step(parameters, opt_state, basis):
        ...
        gradient = grad_fun(jnp.array([1.])[0])[0]
...
if __name__ == "__main__":
    ...
    model_r = models.MLP([1], param_dtype=jnp.complex64, kernel_init=models.complex_kernel_init)
    ...
    gradient = grad_fun(jnp.array([1.+0.j])[0])[0]
    print(f'gradient:\n{gradient}\n')
    #exit()
    ...
    parameters, opt_state = optimize(optimizer, parameters, psi, psi_target, basis, max_iter=5)
    ...
    import hamiltonians, wavefunctions, samplers, driver
    omega = 3.
    lam = 0.5
    g = (lam * omega / 2)**0.5
    ...
    for k in [0]:
        wave = wavefunctions.nn_complex(model_r.apply, model_phi.apply, n_nn_parameters, k=(k,))
        ...
        parameters = driver.driver(walker, ham, parameters, wave, lattice, sampler, n_steps=n_steps, step_size=step_size, seed=seed)