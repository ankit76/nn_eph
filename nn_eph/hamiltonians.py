...
if __name__ == "__main__":
  ...
  np.random.seed(3)
  elec_pos = 1
  n_bonds = n_sites if n_sites > 2 else 1
  phonon_occ = jnp.array(np.random.randint(3, size=n_bonds))
  gamma = jnp.array(np.random.rand(n_bonds // 2 + 1))
  #gamma = jnp.array([ 1. ])
  #parameters = gamma
  #print(f'params: {parameters}')
  model = models.MLP([20, 1])
  model_input = jnp.zeros(n_sites + n_bonds)
