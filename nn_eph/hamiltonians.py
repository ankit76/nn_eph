...
class kq_ne_np:
    ...
    def local_energy_and_update_lr(
        ...
        def scanned_fun(carry, kp):
            ...
            phonon_change = 1
            new_overlap = wave.calc_overlap(
                new_elec_n_k, new_phonon_occ, parameters, lattice
            )
            overlap_ratio = jnp.exp(new_overlap - overlap)
            new_overlap_gradient = wave.calc_overlap_gradient(
                new_elec_n_k, new_phonon_occ, parameters, lattice
            ...
            carry[1] = carry[1].at[m, nu, 2 * kp_i].set(prob_ratio)
            ...
            ratio = (phonon_occ[(nu, *qd)]) ** 0.5 * overlap_ratio
            carry[0] -= (
                jnp.array(self.g_mn_nu_kq)[elec_n, m, nu, kp_i, qc_i]
                * (phonon_occ[(nu, *qd)]) ** 0.5
                * ratio
            )
            prob_ratio = (phonon_occ[(nu, *qd)]) ** 0.5 * new_prob**0.5 / prob**0.5
            carry[1] = carry[1].at[m, nu, 2 * kp_i + 1].set(prob_ratio)
