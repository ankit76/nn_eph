...
from nn_eph import lattices, models, wavefunctions, hamiltonians, samplers, driver, heisenberg
...
for g in [ 0.1, 0.3, 0.5, 0.7, 0.9, 1.1, 1.3 ]:
  ...
  model_phi = models.MLP([1], param_dtype=jnp.complex64, kernel_init=jax.nn.initializers.zeros)
  ...
  wave = wavefunctions.spin_nn_complex(model_r.apply, model_phi.apply, n_nn_parameters, mask=jnp.array([1., 0.]), get_input=wavefunctions.get_input_spin_phonon)
  ...
  walker = [ jnp.array([ 0.5 if i % 2 == 0 else -0.5 for i in range(n_sites) ]), jnp.array([ 0 for _ in range(n_sites) ]) ]
  n_steps = 1000
  step_size = 0.02
